...
// 获取code登录凭证
// 在插件中使用时，需要在用户信息功能页中获得用户授权或满足一定条件后调用。否则将返回 fail。详见 用户信息功能页
...
let code = ''
onLoad(async () => {
  const res = await wx.login() // "0f3hGall2J1rwe4DOaml2MTtRF2hGalf"
...
// 模拟手机号快捷登录  获取手机号功能，目前针对非个人开发者，且完成了认证的小程序开放
const onGetphonenumberSimple = async () => {
  const res = await postLoginWxMinSimpleAPI('18749682618')
  loginSucess(res.result)
}
